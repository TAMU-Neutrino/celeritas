...
class VecgeomTrackView
{
  ...
  public:
    ...
    //! Incidence cosine below which the navigator's own push is too shallow
    static CELER_CONSTEXPR_FUNCTION real_type grazing_cosine() { return 0.1; }

    //! Cap on how far a grazing crossing may be advanced
    static CELER_CONSTEXPR_FUNCTION real_type max_clearance_step()
    {
        return 1e-5;
    }
...
CELER_FUNCTION void VecgeomTrackView::cross_boundary()
{
    ...
    // Advance a grazing crossing clear of the face it just crossed. The
    // navigator moves its evaluation point along the *direction* by a fixed
    // push, so the perpendicular clearance it gains shrinks with the cosine
    // of the incidence angle: below roughly cos = 0.1 the point stays inside
    // the surface tolerance, the same face is reported again on the next
    // step, and surface physics runs a second time on a photon that has
    // already crossed. Advancing along the direction keeps the track on its
    // own trajectory; the distance is scaled to give an angle-independent
    // clearance and capped so an extremely grazing crossing cannot make a
    // physically visible jump. Steeper crossings are left untouched: the
    // navigator's own push already clears them, and an unconditional
    // advance costs light in micron-thin layers.
    if (normal_ != Real3{0, 0, 0})
    {
        real_type const proj = dot_product(dir_, normal_);
        real_type const cos_inc = proj < 0 ? -proj : proj;
        if (cos_inc < this->grazing_cosine())
        {
            axpy(min(this->surface_clearance()
                         / max(cos_inc, real_type{1e-3}),
                     this->max_clearance_step()),
                 dir_,
                 &pos_);
        }
    }
